        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert cryptocurrency trader with access to real-time market data. Provide comprehensive, actionable trading recommendations based on the current market conditions. Be precise with entry points, targets, and stop losses. Consider the actual market data provided and current price levels. Use advanced technical and fundamental analysis."
                    },
                    {
                        "role": "user",
            ...
            if "o3" in str(e) or "invalid" in str(e).lower():
                print("ℹ️ o3 model not available, using GPT-4...")
                response = client.chat.completions.create(
                    model="gpt-4",
                    messages=[
                        {
